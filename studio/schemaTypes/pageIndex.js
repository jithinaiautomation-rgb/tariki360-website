 ...
 * studio/scripts/migrate-home-lists.js moves their contents into the lists.
 */

/* ---------------------------------------------------------------- helpers */

const text = (name, title, description, group, fieldset) =>
  defineField({ name, title, description, type: 'localeString', group, fieldset });

const paragraph = (name, title, description, group, fieldset) =>
  defineField({ name, title, description, type: 'localeText', group, fieldset });

const textList = (name, title, description, group, fieldset) =>
  defineField({
    name,
    title,
    description,
...
const percentField = defineField({
  name: 'percent',
  title: 'Percentage',
 ...
 * (an icon or percentage) is shown next to the language status.
 */
function list({ name, title, description, group, fieldset, itemName, itemTitle, fields, titleKey, plainKey }) {
  const localeKeys = fields.filter((f) => f.type === 'localeString' || f.type === 'localeText').map((f) => f.name);
  return defineField({
    name,
    title,
    description,
...
/** The previous single-box JSON version of a list. */
const oldFormat = (name, group, fieldset) =>
  defineField({
    name,
    title: 'Old format — not used once the list above has rows',
    description:
      'The previous single-box version of the list above. Run scripts/migrate-home-lists.js to move it into the list; that also removes this box.',
    type: 'text',
    rows: 3,
    group,
    fieldset,
